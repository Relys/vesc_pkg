                              ...
                              float speed)        // multiplier for pulse speed
{
    ...
    // Map SoC to red/green
    float red_f   = 255.0f * (1.0f - soc) * pulse;
    float green_f = 255.0f * soc * pulse;

    if (red_f   < 0.0f) red_f   = 0.0f; if (red_f   > 255.0f) red_f   = 255.0f;
    if (green_f < 0.0f) green_f = 0.0f; if (green_f > 255.0f) green_f = 255.0f;

    uint8_t r = (uint8_t)floorf(red_f);
    uint8_t g = (uint8_t)floorf(green_f);
    uint8_t b = 0;

    // Pack as 0x00RRGGBB (swap channels if your pipeline expects GRB)
    uint32_t color = ((uint32_t)r << 16) | ((uint32_t)g << 8) | (uint32_t)b;

    // Only LED 0 ("button"), full blend
    led_set_color(leds, strip, 0, color, 1.0, 1.0f);
}
...
static void status_animate(
    Leds *leds, const LedStrip *strip, float current_time, float blend, float idle_blend, float rpm, float duty_cycle_now, float battery_level, bool is_charging
) {
    if (fabsf(rpm) > ERPM_MOVING_THRESHOLD) {
        leds->status_idle_time = current_time;
    }

    float duty = 0;
    if (leds->state.state == STATE_RUNNING && leds->state.mode != MODE_FLYWHEEL) {
        duty = fminf(fabsf(duty_cycle_now * 10.0f / 9.0f), 1.0f);
    }
    ...
    if(strip->length == 1)
    {
        anim_button_state(leds, strip, battery_level, is_charging, current_time, leds->cfg->status_idle.speed);
    }
...
void leds_setup(Leds *leds, CfgHwLeds *hw_cfg, const CfgLeds *cfg, FootpadSensorState fs_state) {
    ...
    if (led_data) {
        if (led_driver_setup(&leds->led_driver, hw_cfg, strip_array)) {
...
void leds_update(Leds *leds, const State *state, FootpadSensorState fs_state, float pitch, float rpm, float duty_cycle_now, float battery_level, float distance_abs, bool is_charging, float motor_current) {
    ...
    if (leds->headlights_time > 0.0f) {
        ...
        if (hl_split >= 1.0f || (leds->headlights_on == headlights_should && hl_split <= -1.0f)) {
            ...
            if (leds->state.state == STATE_RUNNING) {
                ...
                leds->split_distance = distance_abs;
            }
            reset_led_bars(leds, target_bar(leds, false), target_bar(leds, true), current_time);
        }
    }
    ...
    if (leds->status_strip.length > 0) {
        ...
        status_animate(leds, &leds->status_strip, current_time, 1.0f, leds->status_idle_blend, rpm, duty_cycle_now, battery_level, is_charging);
    }
